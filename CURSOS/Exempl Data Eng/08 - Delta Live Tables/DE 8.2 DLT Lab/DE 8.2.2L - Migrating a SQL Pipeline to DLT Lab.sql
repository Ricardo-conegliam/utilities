...
-- TODO
CREATE STREAMING LIVE TABLE recordings_bronze
...
CREATE OR REFRESH STREAMING LIVE TABLE recordings_enriched
  (CONSTRAINT invalid_heartrate EXPECT (heartrate > 0) ON VIOLATION DROP ROW)
AS SELECT 
  CAST(device_id as INT) as device_id, 
  CAST(r.mrn as LONG) as mrn, 
  CAST(heartrate as DOUBLE) as heartrate, 
  CAST(FROM_UNIXTIME(DOUBLE(time), 'yyyy-MM-dd HH:mm:ss') AS TIMESTAMP) time ,
  name
  FROM STREAM(live.recordings_bronze) r
  INNER JOIN LIVE.pii p
  ON p.mrn = r.mrn
...
CREATE OR REFRESH LIVE TABLE daily_patient_avg
  COMMENT "Golden data from recordings_enriched"
AS SELECT 
   mrn,
   name,
